...
// Helper function to generate page size options (extracted to reduce nesting)
...
export const useTablePageSize = (options: UseTablePageSizeOptions = {}) => {
  ...
  useEffect(() => {
    const calculateOptimalPageSize = () => {
      ...
      // Generate page size options using the extracted helper function
      const filteredOptions = generatePageSizeOptions(optimalRows, minRows, maxRows);
    ...
    globalThis.addEventListener('resize', calculateOptimalPageSize);
    return () => globalThis.removeEventListener('resize', calculateOptimalPageSize);
