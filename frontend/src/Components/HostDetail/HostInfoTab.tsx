  ...
  return (
    <Grid size={{ xs: 12 }}>
      <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
        ...
        {openTelemetryStatus.grafana_configured &&
            ...
            if (openTelemetryStatus) {
              return (
                <Grid container spacing={2}>
                  ...
                  {!openTelemetryStatus.deployed &&
                    hasPermissionSync(SecurityRoles.DEPLOY_OPENTELEMETRY) &&
                    host?.is_agent_privileged && (
                      <Grid size={{ xs: 12 }}>
                        <Button
                          ...
                          disabled={
                            openTelemetryDeploying || openTelemetryLoading
                          }
                        >
                          {openTelemetryDeploying
                            ? t("hostDetail.deploying", "Deploying...")
                            : t(
                                "hostDetail.deployOpenTelemetry",
                  ...
                  {openTelemetryStatus.deployed && (
                    <>
                      <Grid size={{ xs: 12 }}>
                        <Typography variant="body2" color="textSecondary">
                          {t(
                            "hostDetail.opentelemetryServiceStatus",
                            "Service Status",
                          )}
                        </Typography>
                        <Chip
                          label={getOpenTelemetryServiceLabel(
                            t,
                            openTelemetryStatus.service_status,
                          )}
                          color={getOpenTelemetryServiceColor(
                            openTelemetryStatus.service_status,
                          )}
                          size="small"
                        />
                      ...
                      <Grid size={{ xs: 12 }}>
                        <Typography variant="body2" color="textSecondary">
                          {t(
                            "hostDetail.opentelemetryGrafanaServer",
                            "Grafana Server",
                          )}
                        </Typography>
                        <Typography variant="body1">
                          {openTelemetryStatus.grafana_url ||
                            t(
                              "hostDetail.opentelemetryNotConnected",
                              "Not Connected",
                            )}
                        </Typography>
                      </Grid>
        ...
        <CardContent>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <Typography variant="body1">
                {host.ipv4 || t("common.notAvailable")}
              </Typography>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="body2" color="textSecondary">
              ...
              <Typography variant="body1">
                {host.ipv6 || t("common.notAvailable")}
              </Typography>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="body2" color="textSecondary">
              ...
              <Typography variant="body1">
                {host.public_ip || t("common.notAvailable")}
              </Typography>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="body2" color="textSecondary">
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <Chip
                label={
                  host.approval_status.charAt(0).toUpperCase() +
                  host.approval_status.slice(1)
                }
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              >
                ...
                {enabledShells.length > 0 ? (
                  enabledShells.map((shell: string) => (
                    <Chip
                      key={shell}
                      label={shell}
                      color="success"
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <Typography variant="body1">
                {host.agent_version || t("common.notAvailable", "N/A")}
              </Typography>
            </Grid>
          </Grid>
        </CardContent>
        ...
        <CardContent>
          ...
          >
            ...
            <Typography variant="caption" color="textSecondary">
              {t("hosts.updated", "Updated")}:{" "}
              {formatTimestamp(t, host.os_version_updated_at)}
            </Typography>
          </Typography>
          <Grid container spacing={2}>
            <Grid size={{ xs: 12 }}>
              ...
              <Typography variant="body1">
                {host.platform || t("common.notAvailable")}
              </Typography>
            </Grid>
            <Grid size={{ xs: 12 }}>
              <Typography variant="body2" color="textSecondary">
              ...
              <Typography variant="body1">
                {host.platform_release || t("common.notAvailable")}
              </Typography>
            </Grid>
            <Grid size={{ xs: 12 }}>
              <Typography variant="body2" color="textSecondary">
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <Typography variant="body1">
                {host.machine_architecture || t("common.notAvailable")}
              </Typography>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="body2" color="textSecondary">
              ...
              <Typography variant="body1">
                {host.processor || t("common.notAvailable")}
              </Typography>
            </Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <Typography variant="body2" color="textSecondary">
              ...
              <Typography variant="body1">
                {host.timezone || t("common.notAvailable")}
              </Typography>
            </Grid>
            {host.os_details && (
              <Grid size={{ xs: 12 }}>
                ...
                >
                  ...
                  <IconButton
                    ...
                    onClick={() =>
                      handleShowDialog(
                        t(
                          "hostDetail.additionalOSDetails",
            ...
            <Grid container spacing={2}>
                ...
                <>
                  {graylogMechanism && (
                    <Grid size={{ xs: 12 }}>
                      ...
                      <Typography variant="body1">
                        {graylogMechanism === "syslog_tcp" &&
                          t("graylog.mechanism.syslogTcp", "Syslog TCP")}
                        {graylogMechanism === "syslog_udp" &&
                          t("graylog.mechanism.syslogUdp", "Syslog UDP")}
                        {graylogMechanism === "gelf_tcp" &&
                          t("graylog.mechanism.gelfTcp", "GELF TCP")}
                        {graylogMechanism === "windows_sidecar" &&
                          t(
                            "graylog.mechanism.windowsSidecar",
                            "Windows Sidecar",
                          )}
                        {graylogPort &&
                          ` ${t("hostDetail.graylogPort", "(port {{port}})", { port: graylogPort })}`}
                      </Typography>
                    </Grid>
                  )}
              ...
              {!canAttachGraylog && (
                <Grid size={{ xs: 12 }}>
                  <Typography variant="body2" color="textSecondary">
                    {t(
                      "hostDetail.graylogNotConfigured",
                      "Graylog integration not configured or not healthy",
                    )}
                  </Typography>
                </Grid>
              )}
              {canAttachGraylog && !graylogEligible && (
                <Grid size={{ xs: 12 }}>
                  <Typography variant="body2" color="textSecondary">
                    {t(
                      "hostDetail.graylogRequiresPrivileged",
                      "Requires agent in privileged mode",
                    )}
                  </Typography>
                </Grid>
              )}
...
const HostInfoTab: React.FC<HostInfoTabProps> = ({
  ...
  return (
    <Grid container spacing={3}>
      ...
      {/* Config-management run history (Phase 20.1).  Full width:
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 2 }}>
              {hostTags.map((tag) => (
                <Chip
                  ...
                  label={tag.name}
                  onDelete={
                    canEditTags ? () => handleRemoveTag(tag.id) : undefined
                  }
