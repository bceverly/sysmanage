...
def upgrade() -> None:
    ...
    if "audit_log" not in inspector.get_table_names():
        op.create_table(
            ...
            sa.Column(
                "username", sa.String(length=255), nullable=True
            ),  # Denormalized for historical record
            sa.Column(
                "action_type", sa.String(length=50), nullable=False
            ),  # e.g., 'CREATE', 'UPDATE', 'DELETE', 'AGENT_MESSAGE'
            sa.Column(
                "entity_type", sa.String(length=100), nullable=False
            ),  # e.g., 'host', 'user', 'package', 'script'
            sa.Column(
                "entity_id", sa.String(length=255), nullable=True
            ),  # ID of the affected entity
            sa.Column(
                "entity_name", sa.String(length=255), nullable=True
            ...
            ),  # Additional structured data (old/new values, etc.)
            sa.Column(
                "ip_address", sa.String(length=45), nullable=True
            ),  # IPv4 or IPv6
            sa.Column(
                "user_agent", sa.String(length=500), nullable=True
            ),  # Browser/client info
            sa.Column(
                "result", sa.String(length=20), nullable=False
        ...
        op.create_index(
            op.f("ix_audit_log_action_type"), "audit_log", ["action_type"], unique=False
        )
        op.create_index(
