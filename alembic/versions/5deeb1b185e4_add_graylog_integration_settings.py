...
def upgrade() -> None:
    ...
    if "graylog_integration_settings" not in tables:
        ...
        op.create_table(
            ...
            sa.Column(
                "host_id", GUID(), nullable=True
            ),  # References host.id when using managed server
            sa.Column(
                "manual_url", sa.String(255), nullable=True
            ),  # Manual URL when not using managed server
            sa.Column(
                "use_managed_server", sa.Boolean(), default=True, nullable=False
            ),  # True for dropdown, False for manual
            sa.Column(
                "api_token_vault_token", sa.String(255), nullable=True
            ),  # Vault token for Graylog API token
            sa.Column(
                "created_at",
                sa.DateTime(),
                nullable=False,
