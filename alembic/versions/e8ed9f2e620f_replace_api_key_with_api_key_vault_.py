...
def upgrade() -> None:
    ...
    columns = [
        col["name"] for col in inspector.get_columns("grafana_integration_settings")
    ]
    ...
    if "api_key" in columns and "api_key_vault_token" not in columns:
        ...
        op.alter_column(
            ...
            "api_key",
            new_column_name="api_key_vault_token",
        )
        ...
        op.add_column(
            "grafana_integration_settings",
            sa.Column("api_key_vault_token", sa.String(255), nullable=True),
        )
