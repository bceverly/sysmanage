...
def upgrade() -> None:
    ...
    if "default_repository" not in tables:
        ...
        op.create_index(
            "ix_default_repository_os_name", "default_repository", ["os_name"]
        )
        op.create_index(
            "ix_default_repository_package_manager",
            "default_repository",
            ["package_manager"],
        )
        op.create_index(
            "ix_default_repository_os_pm",
            "default_repository",
...
def downgrade() -> None:
    ...
    result = bind.execute(
        sa.text(
            f"SELECT COUNT(*) FROM security_roles WHERE group_id = '{settings_group_id}'"
        )
    )
    if result.scalar() == 0:
        op.execute(f"""
