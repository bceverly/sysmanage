...
try {
    ...
    if ($process.ExitCode -eq 0) {
        ...
        # The MSI continues and the SysManage Server service is
        # registered.  The service will fail to start until Python
        # 3.9+ is on PATH; the operator can install Python manually
        # then restart the service.  Surface a clear message in the
        # install log so the failure mode is diagnosable.
        Write-Log "WARNING: Python installer returned exit code $($process.ExitCode)"
        Write-Log "WARNING: SysManage Server service will not start until Python 3.9+ is installed."
        Write-Log "WARNING: Install Python from https://www.python.org/downloads/ then restart the service:"
        Write-Log "WARNING:   sc.exe start SysManageServer"
        exit 0
    }
