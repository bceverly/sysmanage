...
class TestReportBrandingAuth:
    def test_get_branding_requires_auth(self, client):
        r = client.get("/api/v1/report-branding")
        assert r.status_code in (401, 403)

    def test_put_branding_requires_auth(self, client):
        r = client.put("/api/v1/report-branding", json={"company_name": "X"})
        assert r.status_code in (401, 403)

    def test_logo_upload_requires_auth(self, client):
        r = client.post(
            "/api/v1/report-branding/logo",
...
class TestReportBrandingCrud:
    def test_get_creates_singleton_on_first_read(self, client, auth_headers):
        r = client.get("/api/v1/report-branding", headers=auth_headers)
    ...
    def test_put_updates_company_and_header(self, client, auth_headers):
        ...
        r2 = client.get("/api/v1/report-branding", headers=auth_headers)
...
class TestReportBrandingLogo:
    ...
    def test_get_logo_when_missing_returns_404(self, client, auth_headers):
        # Reset state first.
        client.delete("/api/v1/report-branding/logo", headers=auth_headers)
        r = client.get("/api/v1/report-branding/logo", headers=auth_headers)
    ...
    def test_upload_png_then_get_round_trip(self, client, auth_headers):
        ...
        r = client.post(
            "/api/v1/report-branding/logo", headers=auth_headers, files=files
        )
        assert r.status_code == 200, r.text
        body = r.json()
        ...
        r2 = client.get("/api/v1/report-branding/logo", headers=auth_headers)
    ...
    def test_delete_logo_removes_bytes(self, client, auth_headers):
        ...
        files = {"file": ("logo.png", io.BytesIO(self._PNG_MIN), "image/png")}
        client.post("/api/v1/report-branding/logo", headers=auth_headers, files=files)
        r = client.delete("/api/v1/report-branding/logo", headers=auth_headers)
        assert r.status_code == 200
