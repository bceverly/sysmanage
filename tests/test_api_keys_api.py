...
class TestApiKeyCrud:
    ...
    def test_create_returns_plaintext_once(self, client):
        resp = client.post("/api/v1/api-keys", json={"name": "ci-pipeline"})
    ...
    def test_list_omits_secret(self, client):
        client.post("/api/v1/api-keys", json={"name": "k1"})
    ...
    def test_get_single_key(self, client):
        created = client.post("/api/v1/api-keys", json={"name": "single"}).json()
        resp = client.get(f"/api/v1/api-keys/{created['id']}")
        assert resp.status_code == 200
        assert resp.json()["id"] == created["id"]
        assert "key" not in resp.json()

    def test_get_missing_key_404(self, client):
        resp = client.get("/api/v1/api-keys/00000000-0000-0000-0000-000000000999")
        assert resp.status_code == 404

    def test_revoke_key(self, client):
        created = client.post("/api/v1/api-keys", json={"name": "to-revoke"}).json()
        resp = client.delete(f"/api/v1/api-keys/{created['id']}")
    ...
    def test_revoke_missing_key_404(self, client):
        resp = client.delete("/api/v1/api-keys/00000000-0000-0000-0000-000000000999")
        assert resp.status_code == 404
...
class TestApiKeyManagementGuard:
    ...
    def test_revoke_forbidden_with_api_key_auth(self, client):
        created = client.post("/api/v1/api-keys", json={"name": "guarded"}).json()
        resp = client.delete(
