...
- **SonarQube/SonarCloud scans are issue-free** -- 0 new bugs, 0
  ...
  **Reading the SonarCloud gate badge:** judge this item on *findings*
  ...
  review, NOT on the raw `projectStatus` color.  One of SonarCloud's own
...
- Open-source scripting and fleet endpoints continue to work (no regression in free-tier behavior after the migration)
...
6. [x] Migrate the `tick` hook caller (the external systemd timer / cron) -- there is no first-party scheduler shipped with sysmanage; deployments wire their own.  The only behavior change for existing callers is that `/api/upgrade-profiles/tick` now returns 402 unless `automation_engine` is loaded.
...
- [x] Enterprise-wide update policy management (define policies centrally, push to sites) -- June 2026: both `firewall_role` AND `update_profile` now materialize locally (`update_profile` → the `upgrade_profiles` table via `federation_policy_apply_service.apply_update_profile`); push + inbox + apply-worker path complete
- [x] Enterprise-wide firewall role management (define roles centrally, push to sites) -- end-to-end June 2026: coordinator push worker → site inbox → `federation_policy_apply_service` materializes into local `firewall_role` + `firewall_role_open_port`
...
- [x] Configurable sync intervals per site (bandwidth-constrained sites can sync less frequently) -- `sync_interval_seconds` on `create_site` / `update_site`, persisted per `FederationSite` and honored by the site engine tick
...
chips color-code by site state (enrolled / pending / suspended);
...
Leaflet + OSM tiles -- DivIcon markers colored by status,
...
- [x] **Tile dashboard view** -- landed (June 2026):
      ...
      a status-colored tile grid below (no geography), each tile click →
...
1. **[Deferred -- not load-bearing]** Move `AccessGroup`,
   ...
   user-visible behavior change.  Architectural purity vs. cost
...
**Current state (re-measured 2026-05-08 after autonomous translation pass):**
  ...
  6. **Pro+ engine plan descriptions** -- 360 hardcoded English
     ...
     **SUPERSEDED (2026-06-22) -- the ``{key, params}`` envelope below
     was retired in favor of Model A gettext.**  The envelope approach
...
got hammered, a malformed payload chewed CPU forever.  Now:
  ...
  * `peek_batch` (sync queue), `list_all_pending_pushes`
    ...
    (commands) all honor the backoff window -- entries fail and
...
loop -- the next tick honors the backoff naturally.  Sync worker
...
- [x] Every capability 402-clean when unlicensed; Pro+ can author templates but not provision -- 2026-08-04: the provisioning surface had NO stubs at all, so an unlicensed server answered 404 (indistinguishable from a bad URL) for both the 18.1 compute endpoints and the 18.2 bare-metal ones. `_mount_provisioning_stubs` now answers 402 with a license message across providers/compute-resources/templates/jobs/provision and install-sources/install-assignments/discovered-hosts/bootdisk/status. These return 402 rather than the `{"licensed": false}` HTTP 200 the other stub groups use, deliberately: that shape exists so an OSS-tier page can render a license prompt, but the provisioning UI ships only in the Pro+ plugin -- the callers here are scripts, and answering a POST that would assign a machine an OS with 200 would read as success. Covered by `TestProvisioningStubs`.
- [x] **Phase exit gate** (see [Phase Exit Gate](#phase-exit-gate-mandatory-final-item-for-every-phase)): all tests pass · lint issue-free · no performance regressions · SonarQube scans issue-free -- 2026-08-04
...
- [x] **Report WHY a provisioned host failed, from the host.** A machine
      *(DONE 2026-08-12 -- the bootstrap tees its output to /var/log/sysmanage-bootstrap.log, probes for the agent binary, and POSTs {state, detail, log_tail}. New `agent_missing` state distinguishes "the script ran" from "the agent is up"; it DISARMS netboot exactly like `installed`, because re-arming a machine whose OS is fine would erase a working system, while `failed` stays armed for a retry. Stored on the assignment, serialized by `to_dict`, and surfaced in Provisioning → Bare Metal with the reason inline and the log tail in a dialog. Docs + screenshot + 13-locale i18n shipped with it.)*
...
- [x] **Make the bare-metal harness cheap enough to run casually.** Most of the
      ...
      *(DONE 2026-08-21 -- the remaining two halves landed as
      ...
      behavior change is yours and not the archive's; `--max-age` opts into
      ...
      path traversal is REFUSED rather than normalized.
...
- [x] **One port to rule them all: agent↔server needs 443 only.** A customer
      ...
      works" is the whole point:
        ...
        * **HTTP CONNECT proxies**, including authenticated ones -- honor the
      ...
      *(DONE 2026-08-14 -- the premise was half wrong, which changed the work.  The
      ...
      still honored so no deployed agent is stranded.  `ca_bundle` for
      ...
      anyone in the path could impersonate the server and enroll the host into their
...
- [x] **The Windows MSI cannot bootstrap Python, and reports success anyway.**
      ...
      **bundle** (`sysmanage-agent-bundle.wxs`) chains VC++ -> Python -> the agent
      ...
      installer mutex; its no-argument behavior is unchanged so
...
- [x] Docs + 14-language i18n complete
      ...
      * `docs/professional-plus/index.html` -- new "How Modules Are Verified"
      ...
      `available_packages` delta (protocol optimization, invisible to
      ...
      carries a distinct border AND heading color so the distinction survives
      ...
      *(MEASURED 2026-08-25 -- two of the four sub-targets are ALREADY MET, and
      ...
      suites, 972 -> 1041 tests, all green, eslint clean:
        * `hostDetailHelpers.test.ts` (50 tests) -- the pure helpers.  Worth the
          ...
          behavior that is easy to break silently, e.g. gid 0 must read as
      ...
      Finished with the last two pages:
        ...
        * `Secrets.test.tsx` (7 tests) -- pins that `{ licensed: false }` is
          recognized as a shape rather than treated as a secret, and that an
...
single-host apply. Remaining work, in the order it should be done:
  ...
  2. ~~Per-engine refactor (agent + server + UI) and licensing split~~ --
     **DONE 2026-08-27.** `ConfigManagementEnginesCard` replaced the
     ...
     one applies, installed engines first, licensed adapters labeled
  ...
  4. **Pro+ `config_management_engine`** -- profile authoring/storage,
     ...
     2026-08-27:**
       ...
       * CRUD + assignment REST API -- `backend/api/config_mgmt_profiles.py`,
         ...
         default.
       * Profile authoring UI -- `/config-profiles`, license-gated at the route
       ...
       * Engine catalog endpoint -- `GET /config-management/engines`, host
         ...
         of a second copy in the UI. Not license-gated: knowing that "puppet"
         is a word is worthless without the adapter, and an empty dropdown on
         a page already behind the license reads as a bug.
...
**Agent-side foundations -- LANDED 2026-08-26.**
  ...
  **Validated against real ansible-core 2.21.3, not just unit-tested.** Fresh
  ...
  status and exit code still produced the right verdict -- defense in depth
...
group → a `capability_probes` entry.
  ...
  * The probe needed a **new table**, `_REQUIRED_LOCATORS`, because `dsc.exe`
    ...
    `lookup` AND `system` the tables use; honoring one but not the other
...
Three behaviors were **observed, not assumed**, and each would have been a
production defect if guessed:
  1. `dsc` writes ANSI-colored logs to **stderr** and results to stdout;
...
OSS-side is done end to end:
  ...
  * `ConfigProfileRunHistory` panel. **"Changed" and "No changes" are visually
    ...
    are labeled so they can never be mistaken for an applied change.
      ...
      **LICENSING -- DECIDED 2026-08-27 (Bryan).** `config_management_engine` is
      ...
      "config management is Enterprise":
        ...
        * **Enterprise gets the Puppet/Salt/Chef adapters**, plus profile
          ...
          MIGRATION BRIDGE for an organization -- the classic thing you sell
...
* **S3 -- Alert rule condition.** `AlertRule.condition_type` is an open string
  ...
  Pro+ change, a module rebuild and a fresh license before it can be tested.
...
provider does not have to emulate a query engine -- it materializes the
...
- [x] **S2 -- Native provider, and it ships FIRST.** -- 2026-09-21 Materialise the v1 tables
      ...
      **Shipped 2026-09-21 -- all 16 contract tables have a native provider.**
      `core/fact_store.py`: contract tables materialized into in-memory SQLite
...
- [x] **S3 -- osquery provider.** -- 2026-09-21 Embed and lifecycle-manage `osqueryd`
      ...
      **Deliberately NOT done in S3, each with a reason rather than an
      ...
      platform and would put osquery's license and CVE surface into every
...
- [x] **S4 -- Query packs as multi-tenant policy.** -- 2026-09-21 Curated/shipped
      ...
      **NEW Pro+ engine `query_pack_engine`** (Professional tier), registered
      ...
      `PROFESSIONAL_FEATURES` list, which is what the license-generation GUI
      reads, so `query_pack_engine` would have been offered as a module while
      `query_pack_manage` could never appear in any issued license. Caught by
      ...
      and authorization and re-implements none of them. 43 tests, built abi3.
      ...
      **Agent (`run_query_pack`, Community).** `collection/query_pack_runner.py`
      materializes ONLY the declared tables -- collecting all sixteen for a
      ...
      this slice. Work runs in the executor -- materializing tables is seconds
      ...
      **Licensing gate extended, after this slice drifted three ways at once.**
      ...
      `multiuser` are issued in EVERY Professional license and were absent
      from `FeatureCode` entirely, so `FeatureCode(f)` over a real license's
      feature list raised on a perfectly valid license. Never bit only because
      ...
      **Also fixed here, found by the full Pro+ run:** `agent_install.pxi`
      ...
      over each mirror's PROPRIETARY license line while not fixing anything.
      ...
      **Shipped broken, found on first page load, fixed same day.** All seven
      ...
      traverse a route. Reads now rely on the router's JWT + license gate, as
      ...
      **Needs Bryan (license-server side):** `query_pack_engine` is new, so the
      ...
      unlicensed behavior, not a fault.
- [x] **S5 -- Ad-hoc fleet-wide live query + results surface** -- 2026-09-22,
      ...
      **What the slice actually is: the BOUND.** An ad-hoc query is one
      ...
      loses its license mid-query stops dispatching rather than reverting to
      the behavior this exists to prevent.
...
- [x] **S6 -- Wire the tables into the consuming engines** -- `compliance_engine`
      ...
      **Engines are prebuilt binaries, so the bundle CANNOT simply be passed.**
      The `.so` files come from the license server and lag the OSS code; all 22
      ...
      "stale runtime engine" failure that caused the "No tenant" PXE enroll bug.
...
- [x] **S7 -- Extend golden-host drift to arbitrary file / config state**, the
      ...
      **Hash and metadata only; no file CONTENT is collected, stored or
      ...
      private key or a license file without those bytes entering the server
      ...
      **The management plane, completed 2026-09-22.** A collection tick that
      ...
      license gate as the differ it feeds). Two dispatch refusals matter,
...
holding PHI, and a defense subcontractor get the same recommendations today,
...
the network segment of interest, so neighbor discovery is a normal capability
...
**Passive first, active second -- and active is opt-in.** ARP cache, neighbor
