...
async def create_host_user(  # NOSONAR
    ...
    if not current_user.has_role(SecurityRoles.ADD_HOST_ACCOUNT):
        raise HTTPException(
            status_code=403,
    ...
    # Queue the message for delivery to the agent (tenant-scoped data)
    ...
    # Audit log the user creation request (on the main engine)
...
async def create_host_group(
    ...
    # (user/role data is server-global); the audit trail also stays on the main
    # engine, while the host/queue data routes to the tenant engine via ``db``.
    if not current_user.has_role(SecurityRoles.ADD_HOST_GROUP):
        raise HTTPException(
            status_code=403,
    ...
    # Audit log the group creation request (on the main engine)
    from backend.services.audit_service import (
        ActionType,
        AuditService,
        EntityType,
...
async def delete_host_user(
    ...
    # (user/role data is server-global); the audit trail also stays on the main
    # engine, while the host/queue data routes to the tenant engine via ``db``.
    if not current_user.has_role(SecurityRoles.DELETE_HOST_ACCOUNT):
        raise HTTPException(
            status_code=403,
    ...
    # Audit log the user deletion request (on the main engine)
    from backend.services.audit_service import (
        ActionType,
        AuditService,
        EntityType,
...
async def delete_host_group(
    ...
    # (user/role data is server-global); the audit trail also stays on the main
    # engine, while the host/queue data routes to the tenant engine via ``db``.
    if not current_user.has_role(SecurityRoles.DELETE_HOST_GROUP):
        raise HTTPException(
            status_code=403,
    ...
    # Audit log the group deletion request (on the main engine)
    from backend.services.audit_service import (
        ActionType,
        AuditService,
        EntityType,
