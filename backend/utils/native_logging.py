"""Platform-native log handler selection for the SysManage server.

Returns an OS-native ``logging.Handler`` so the server integrates with the
host's logging system — systemd journal (``journalctl``), syslog, or the Windows
Event Log — in addition to its rotating file log.  Opt-in via the ``logging``
section of ``sysmanage.yaml``; returns ``None`` (the caller keeps file logging
and prints a notice) when the requested sink is unavailable, so a missing
optional dependency never stops the server from starting.
"""
...
def _syslog_remote_handler(
    ...
    caller keeps file logging, so a bad remote target never stops the server.
    """
    if not host:
        return None
    try:
