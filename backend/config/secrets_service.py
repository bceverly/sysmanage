...
# Negative caching: distinguishes "fetched, got nothing" from "not fetched
...
# config bag, and a cache would have to be keyed per tenant.
# ---------------------------------------------------------------------------

_TENANT_SECRET_SUBPATH = "sysmanage/tenant"  # nosec B105


def _tenant_secret_path(tenant_id: str) -> str:
    """KV-v2 read/write path for a tenant's consolidated config secret."""
    mount = config.get_vault_mount_path()
    return f"{mount}/data/{_TENANT_SECRET_SUBPATH}/{tenant_id}/config"
...
def get_tenant_secret(
    ...
    *,
    ...
    """Return tenant-scoped secret ``name`` for ``tenant_id``; ``default`` on miss.

    Best-effort and never raises: returns ``default`` if vault is disabled,
    unreachable, or the secret is absent.
    """
    if not config.is_vault_enabled():
        return default
    try:
        ...
        data = VaultService().retrieve_secret(_tenant_secret_path(tenant_id))
        ...
        # nosemgrep: python.lang.security.audit.logging.logger-credential-leak.python-logger-credential-disclosure
        logger.debug("OpenBAO tenant secret unavailable for %s: %s", tenant_id, exc)
        return default


def store_tenant_secrets(tenant_id: str, secrets: dict) -> bool:
    """Write/merge ``secrets`` into ``tenant_id``'s OpenBAO config secret.
    ...
    if not config.is_vault_enabled():
        # nosemgrep: python.lang.security.audit.logging.logger-credential-leak.python-logger-credential-disclosure
        logger.warning("Cannot store tenant secrets: vault is disabled.")
        return False
    try:
        from backend.services.vault_service import VaultService  # noqa: PLC0415
        ...
        svc = VaultService()
        try:
            existing = svc.retrieve_secret(_tenant_secret_path(tenant_id))
            if not isinstance(existing, dict):
                existing = {}
        except Exception:  # noqa: BLE001 - absent secret is fine
            existing = {}
        merged = {**existing, **{k: v for k, v in secrets.items() if v is not None}}
        svc._make_request(  # pylint: disable=protected-access
            "POST", _tenant_secret_path(tenant_id), {"data": merged}
        )
        return True
    except Exception as exc:  # noqa: BLE001
        # nosemgrep: python.lang.security.audit.logging.logger-credential-leak.python-logger-credential-disclosure
