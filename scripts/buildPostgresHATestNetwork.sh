...
# sysmanage, sysmanage-agent, or PostgreSQL — that's your job after the VMs come
# up; the post-start summary prints every command you need.
#
...
print_vm_summary() {
  ...
  case "$role" in
    ...
    pg-standby)
      ...
      echo "              sudo -u postgres rm -rf /var/lib/postgresql/<ver>/main"
      echo "              sudo -u postgres bash -c 'PGPASSWORD=${REPL_PASSWORD} pg_basebackup \\"
      echo "                   -h ${PGP_IP} -U ${REPL_USER} -D /var/lib/postgresql/<ver>/main -R -P'"
      echo "              sudo systemctl start postgresql"
      echo "            (or just run '$0 failback' from the host — it does this for you)"
      echo "  promote : to test failover ->  sudo -u postgres pg_ctlcluster <ver> main promote"
      ;;
    agent)
...
cmd_start() {
  ...
  echo "       ssh ${USERNAME}@${PGS_IP} 'sudo -u postgres pg_ctlcluster <ver> main promote'"
