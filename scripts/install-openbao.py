...
def check_openbao_installed():
    ...
    if system == "windows":
        ...
        potential_paths = [
            os.path.join(
                os.path.expanduser("~"), "AppData", "Local", "bin", binary_name
            ),
    ...
    for path in potential_paths:
        if os.path.isfile(path) and os.access(path, os.X_OK):
            try:
                ...
                if result.returncode == 0:
                    print(
                        f"OpenBAO already installed at {path}: {result.stdout.strip()}"
                    )
                    # Add to PATH hint if not already accessible
                    if system == "windows":
                        parent_dir = os.path.dirname(path)
                        print(
                            f"Note: Add {parent_dir} to your PATH to use 'bao' command globally"
                        )
                    return True
            except Exception:
                continue
...
def install_via_package_manager():
    ...
    try:
        if system == "darwin":
            ...
            result = subprocess.run(
                ["brew", "install", "openbao"], capture_output=True, text=True
            )
            if result.returncode == 0:
            ...
            if os.path.exists(built_binary):
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        os.chmod(
                            target_path, 0o755
                        )  # lgtm[py/overly-permissive-file]  nosemgrep: python.lang.security.audit.insecure-file-permissions.insecure-file-permissions
            ...
            if shutil.which("vault"):
                ...
                try:
                    subprocess.run(
                        ["sudo", "ln", "-sf", "/usr/pkg/bin/vault", "/usr/pkg/bin/bao"],
                        capture_output=True,
                        text=True,
                        check=False,
                    )
            ...
            print(
                "Would you like to build OpenBAO from source? This requires Go and Git."
            )
            ...
            if not shutil.which("go"):
                ...
                print("")
                print(
                    "Note: OpenBAO is optional - the system works without it (vault.enabled=false)"
                )
                return False
            ...
            try:
                ...
                if response in ["y", "yes"]:
                    ...
                    build_script = os.path.join(
                        os.path.dirname(__file__), "build-openbao.sh"
                    )
                    result = subprocess.run(
                        ["sh", build_script], capture_output=False, text=True
                    )
                    ...
                    if result.returncode == 0:
                        ...
                        print(
                            "Build failed. You can try building manually or use vault.enabled=false"
                        )
                        return False
                else:
                    print("Skipping OpenBAO build.")
                    print(
                        "The development environment will work fine with vault.enabled=false"
                    )
                    return False
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if version_match:
                        ...
                        if (
                            version_parts[0] < required_parts[0]
                            or (
                                version_parts[0] == required_parts[0]
                                and version_parts[1] < required_parts[1]
                            )
                            or (
                                version_parts[0] == required_parts[0]
                                and version_parts[1] == required_parts[1]
                                and version_parts[2] < required_parts[2]
                            )
                        ):
                            print(
                                f"Note: Latest OpenBAO prefers Go 1.24.6+, but trying with {current_version}"
                            )
                            print(
                                "Will attempt to build an older compatible version..."
                            )
            except Exception as e:
...
def install_from_binary():
    ...
    try:
        ...
        if platform_str.startswith("netbsd"):
            print(
                "NetBSD precompiled binaries are not available from OpenBAO releases."
            )
        ...
        if platform_str.startswith("openbsd"):
            ...
            print(f"OpenBSD: using pinned, verified version {version}")
        else:
            try:
                import json
        ...
        if platform_str.startswith("windows"):
            ...
            possible_filenames.append(f"openbao_{ver_clean}_linux_{arch}.deb")
            possible_filenames.append(f"openbao_{ver_clean}_linux_{arch}.rpm")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for filename in possible_filenames:
                ...
                try:
                    ...
                    if not (
                        ...
                        and "/openbao/openbao/releases/download/" in parsed_url.path
                    ):
                        raise ValueError(
                            "Invalid download URL - not from official OpenBAO releases"
                        )
            ...
            if downloaded_file.endswith(".deb"):
                ...
                binary_name = (
                    "bao.exe" if platform.system().lower() == "windows" else "bao"
                )
                ...
                if install_path:
                    ...
                    try:
                        ...
                        print(
                            "You may need to run with appropriate privileges or install manually"
                        )
                        ...
                        if alt_path != install_path:
                            ...
                            try:
                                ...
                                print(
                                    f"Add {alt_path} to your PATH to use 'bao' command"
                                )
                                return True
...
def install_deb_package(deb_file):
    ...
    try:
        ...
        result = subprocess.run(
            ["sudo", "dpkg", "-i", deb_file], capture_output=True, text=True
        )
        if result.returncode == 0:
            ...
            if "sudo:" in result.stderr or "password" in result.stderr.lower():
                print(
                    "Cannot use sudo in this environment. Trying manual extraction..."
                )
...
def extract_from_deb(deb_file):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run(
                ["ar", "x", deb_file], cwd=temp_dir, capture_output=True, text=True
            )
            if result.returncode != 0:
...
def main():
    ...
    if system == "openbsd":
        ...
        if install_from_binary():
            ...
            if install_path and check_openbao_at_path(install_path):
                print("\nOpenBAO installation completed successfully!")
                print(
                    f"Note: OpenBAO installed to {install_path} but not in current PATH"
                )
                return 0
        print("\nPrebuilt unavailable - falling back to source build...")
    ...
    if install_from_binary():
        ...
        if install_path and check_openbao_at_path(install_path):
            ...
            print(
                f"Add {install_path} to your PATH or use the full path: {os.path.join(install_path, 'bao')}"
            )
            return 0
        else:
